...
class Project {
  ...
  void initializeDisplays() {
    ...
    //if (screens.isEmpty()) {
    //  screens.add(new Screen(0)); // Default size; must review it later
    //}
    ...
    if (file.exists()) {
      try {
        ...
        for (XML screenXML : screenXMLs) {
          ...
          screens.add(new Screen(screenXML));
        }
  ...
  // Create a new screen with default parameters
  void addNewScene() {
    int newId = screens.get(currentSelectedScreenIndex).scenes.size();
    screens.get(currentSelectedScreenIndex).addScene(newId); // Default size
    //screens.get(currentSelectedScreenIndex).scenesXML.addChild(newScene);
    //saveToFile();
  ...
  void AssignDisplay(Rectangle b) {
    if (screens.get(currentSelectedScreenIndex).isAssigned == false) {
      for (Screen screen : screens) {
        if (screen.x == b.x) {
          screen.unassign();
        }
      }
      screens.get(currentSelectedScreenIndex).assignToDisplay(b);
    } else {
      screens.get(currentSelectedScreenIndex).unassign();
    }
  }
  ...
  void drawUI() {
    ...
    drawStagePanel(currentSelectedScreenIndex); // Pass the index here
