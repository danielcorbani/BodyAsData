...
class Project {
  ...
  void initXMLconfig() {
    ...
    if (file.exists()) {
      try {
        ...
        //println("number of screens (XML): " + screensXML.length);
        ...
        //println("number of scenes (XML): " + scenesXML.length);
        for (XML sceneXML : scenesXML) {
          ...
          //println("number of medias (XML): " + mediasXML.length);
  ...
  void selectScene(int index) {
    if (index >= 0 && index < scenes.size()) {
      ...
      for (Screen screen : screens) {
        ...
        //screen.updateMediaList(scenes.get(currentScene).mediaItems);
      }
    }
  }
  ...
  public void moveHoverPoint(float mousex, float mousey) {
    ...
    //XML mediaItem = config.getChild("Scenes/Scene/Medias/MediaItem");
  }
